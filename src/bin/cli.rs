...
fn build_tx_no_change(coins: Vec<Coin>, recipient: ScriptBuf, kp: Keypair, fee_rate: FeeRate) -> Transaction {
    let inputs = coins
        .iter()
        .map(|coin| TxIn {
    ...
    let output = TxOut { value: send_amount, script_pubkey: recipient.clone() };
...
fn build_tx_with_change(coins: Vec<Coin>, target: Amount, recipient: ScriptBuf, change_addr: ScriptBuf, kp: Keypair, fee_rate: FeeRate) -> Transaction {
    let inputs = coins
        .iter()
        .map(|coin| TxIn {
            previous_output: coin.outpoint,
    ...
    //let mut tx_test = Transaction {
        //version: Version::TWO,
        //lock_time: LockTime::ZERO,
        //input: inputs,
        //output: vec![output.clone(), output],
    //};

    // now that the weight is known, the fee can be calculated.
    //let fee: Amount = FeeRate::fee_wu(fee_rate, tx_test.weight()).unwrap();
    //panic!("fee {:?} weight {:?}", fee, tx_test.weight());
    ...
    println!("send amount {:?} target {:?} change {:?}", value, target, change);

    let output = TxOut { value: target, script_pubkey: recipient.clone() };
    let change_output = TxOut { value: change, script_pubkey: change_addr };

    tx.output = vec![output, change_output];
    println!("final weight {:?}", fee, tx.weight());
...
fn main() {
    ...
    match cli.commands {
        ...
        Commands::Wallet(WalletCmd::SpendUtxo {
            ...
            let tx = build_tx_no_change(coin_vec, address.script_pubkey(), kp, bitcoin_fee_rate);
        ...
        Commands::Wallet(WalletCmd::SpendUtxos {
            ...
            let address: Address =
                Address::from_str(&addr).unwrap().require_network(Network::Signet).unwrap();
            let tx = build_tx_no_change(outs, address.script_pubkey(), kp, bitcoin_fee_rate);
            let builder = Builder::new("https://blockstream.info/signet/api");
            let blocking_client = builder.build_blocking();
            let response = blocking_client.broadcast(&tx).unwrap();
            println!("{:#?}", response);
        ...
        Commands::Wallet(WalletCmd::Spend {
            ...
            let (_, selection) =
                select_coins(total_target, cost_of_change, bitcoin_fee_rate, lt_fee_rate, &coins)
                    .unwrap();
            ...
            let tx = build_tx_with_change(to_spend, target, address.script_pubkey(), sender_script_pub_key, kp, bitcoin_fee_rate);
            //let inputs = tx.input;
            //let outputs = tx.output;
            //println!("input count: {:?} output count: {:?}", inputs.len(), outputs.len());
            //for o in outputs {
                //println!("output val: {:?}", o.value);
            //}
            let builder = Builder::new("https://blockstream.info/signet/api");
            let blocking_client = builder.build_blocking();
            let response = blocking_client.broadcast(&tx).unwrap();
